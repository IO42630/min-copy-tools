...
public class CopyU {
    ...
    public static void copy(CFilePair pair) {
        try {
     ...
     */
    private static void handleSameFile(CFilePair pair) throws IOException {
     ...
     */
    private static void handleModifiedFile(CFilePair pair) throws IOException {
        var src = pair.getSrcFile();
        var dst = pair.getDstFile();
        if (src.lastModified() > dst.lastModified()) {
